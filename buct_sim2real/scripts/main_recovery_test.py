...
class buct_sim2real:
    def __init__(self):
        ...
        try:
            ...
            self.tf_listener.waitForTransform('/map', '/grasp_position5', rospy.Time(), rospy.Duration(2.0))
    ...
    def get_mission(self):
        ...
        for target in self.target_list.target_list:
            #rospy.loginfo('position y:  '+str(target.pose.position.y))
            if ((target.id not in mission_id) and (target.pose.position.y<=-0.1)  and (target.id!='B') and (target.id!='O') and (target.id!='X')):
                mission.append(target)
                mission_id.append(target.id)
            if ((target.id not in mission_id) and (target.pose.position.y<=-0.1) and (target.id=='O')):
                target.id = '3'
                mission.append(target)
                mission_id.append(target.id)
        for j in range(len(mission)-1): # sort
            for i in range(len(mission)-1):
                if(mission[i].pose.position.x > mission[i+1].pose.position.x):
                    temp_target = Target()
                    temp_target = mission[i]
                    temp_target_id = mission_id[i]
                    mission[i] = mission[i+1]
                    mission_id[i] = mission_id[i+1]
                    mission[i+1] = temp_target
                    mission_id[i+1] = temp_target_id
        return mission_id
    ...
    def move_position(self, t_vector):
        ...
        move_base_msg_x.linear.x = x_moveself.min_vel
    ...
    def ifthere(self):
        ...
        for i in self.target_list.target_list:
            if(i.id == self.id):
                rospy.loginfo("TRUETRUETRUE")
                return True
        rospy.loginfo("NOT in the list")
        return False
    ...
    def execute_mission(self,cube_id):
        if(cube_id == '1'):
            self.goto(0.5966, 3.1668, 180.0000) # cube number 1
        elif(cube_id == '2'):
            self.goto(0.0095, 2.6878, -55.0000) # cube number 2
        elif(cube_id == '3'):
            self.goto(1.7032, 2.7603, 0.0000) # cube number 3
        elif(cube_id == '4'):
...
if __name__ == "__main__":
    ...
    while(1):
        ...
        if(tantianwei.state == 0):
            ...
            rospy.sleep(0.1)
            mission = tantianwei.get_mission()           # attempt to get missions, first try
            rospy.loginfo(str(tantianwei.get_mission()))
            if(len(mission)!=3):                         # if first try failed:
                tantianwei.goto(0.3000, 1.5000, -5.0000)
                rospy.sleep(0.1)
                mission = tantianwei.get_mission()       # attempt to get missions, second try
                rospy.loginfo(str(tantianwei.get_mission()))
            if(len(mission)!=3):                         # if second try failed:
                tantianwei.goto(0.2000, 1.4000, -5.0000)
                rospy.sleep(0.1)
                mission = tantianwei.get_mission()       # attempt to get missions, third try
                rospy.loginfo(str(tantianwei.get_mission()))
            tantianwei.state = 1

        # grasp and place the first cube
        elif(tantianwei.state == 1):
            tantianwei.execute_mission(mission[0])
        ...
        # grasp and place the second cube
            ...
            tantianwei.execute_mission(mission[1])
        ...
        # grasp and place the third cube
            ...
            tantianwei.execute_mission(mission[2])
